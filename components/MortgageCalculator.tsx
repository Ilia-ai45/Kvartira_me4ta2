...
({label, value, min, max, step, currentValue, onChange}) => (
    <div>
        <label className="block font-semibold text-gray-700 dark:text-gray-300 mb-2">{label}</label>
        <input type="text" value={value} readOnly className="w-full bg-gray-100 dark:bg-zinc-700 border-gray-300 dark:border-zinc-600 rounded-lg text-lg p-3 font-bold text-gray-900 dark:text-white text-right mb-2 shadow-sm" />
...
const MortgageCalculator: React.FC<MortgageCalculatorProps> = ({
    ...
    return (
        <section id="calculator" className="py-24 bg-gray-100 dark:bg-zinc-900">
            <div className="container mx-auto px-6">
                <div className="text-center max-w-3xl mx-auto mb-16 animate-reveal anim-fade-up">
                    <h2 className="text-3xl sm:text-4xl font-extrabold text-gray-900 dark:text-white mb-4">Ипотечный калькулятор</h2>
                    <p className="text-base sm:text-lg text-gray-600 dark:text-gray-400">
                        Рассчитайте ежемесячный платеж, чтобы оценить свои возможности и быть увереннее в выборе.
                    </p>
                </div>

                <div className="max-w-5xl mx-auto grid lg:grid-cols-5 gap-8 bg-white dark:bg-zinc-800 p-4 md:p-8 rounded-2xl shadow-lg border border-gray-200 dark:border-zinc-700">
                    <div className="lg:col-span-3 space-y-6 animate-reveal anim-slide-right">
                        ...
                        <CalculatorInput label={`Первоначальный взнос (${downPaymentPercentage.toFixed(0)}%)`} value={formatCurrency(downPayment)} min={0} max={propertyPrice} step={50000} currentValue={downPayment} onChange={handleDownPaymentChange} />
                         ...
                         <div>
                            <label className="block font-semibold text-gray-700 dark:text-gray-300 mb-2">Процентная ставка, %</label>
                            <input type="number" step="0.1" value={interestRate} onChange={e => setInterestRate(Number(e.target.value))} className="w-full bg-gray-100 dark:bg-zinc-700 border-gray-300 dark:border-zinc-600 rounded-lg text-lg p-3 font-bold text-gray-900 dark:text-white text-right shadow-sm" />
                        </div>
                        <div className="pt-2">
                            <label className="flex items-center space-x-3 cursor-pointer p-3 rounded-lg bg-gray-200/50 dark:bg-zinc-900/50 hover:bg-gray-200/80 dark:hover:bg-zinc-900/80 transition-colors">
                                <input 
                                    type="checkbox" 
                                    checked={quickDealDiscount} 
                                    onChange={e => setQuickDealDiscount(e.target.checked)} 
                                    className="form-checkbox h-5 w-5 text-amber-500 bg-gray-300 dark:bg-zinc-600 border-gray-400 dark:border-zinc-500 rounded focus:ring-amber-500" 
                                />
                                <span className="text-gray-800 dark:text-white font-semibold">минус 100 000 руб за быстрый выход на сделку</span>
                            </label>
                        </div>
                    </div>

                    <div className="lg:col-span-2 bg-gray-800 dark:bg-zinc-950 text-white p-8 rounded-xl flex flex-col justify-center text-center shadow-2xl border border-amber-500/30 animate-reveal anim-slide-left" style={{'--delay': '0.2s'} as React.CSSProperties}>
